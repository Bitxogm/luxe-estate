...
function buildPageUrl(page: number, priceType?: string, type?: string, city?: string): string {
...
export default async function NewInMarket({ priceType, type, city, page = 1 }: NewInMarketProps) {
    ...
    <section>
      ...
      {meta.totalPages > 1 && (
        <div className="mt-12 flex items-center justify-center space-x-2">
          <a
            href={buildPageUrl(meta.page - 1, priceType, type, city)}
          ...
          <div className="hidden space-x-2 sm:flex">
            {Array.from({ length: meta.totalPages }).map((_, i) => (
              <a
                key={i}
                href={buildPageUrl(i + 1, priceType, type, city)}
